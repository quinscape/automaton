...
public class InteractiveQueryPagingTest
{
    @BeforeEach
    public void init()
    {
    ...
    public void testPagingQuery() throws IOException
    {
        final DSLContext dslContext = TestProvider.create(ImmutableMap.of(
            "select \"foo\".\"id\", \"foo\".\"name\", \"foo\".\"created\", \"owner\".\"id\", \"owner\".\"login\", " +
                ...
                "\"owner\" on \"owner\".\"id\" = \"foo\".\"owner_id\" order by \"foo\".\"id\" offset ? rows fetch " +
                "next ? rows only", (dsl, ctx) -> {
            ...
            "select count(*) from \"public\".\"foo\" as \"foo\" left outer join \"public\".\"app_user\" as \"owner\" on \"owner\".\"id\" = " +
                "\"foo\".\"owner_id\"", (dsl, ctx) -> new MockResult[]{
                new MockResult(
                    dsl.newRecord(
