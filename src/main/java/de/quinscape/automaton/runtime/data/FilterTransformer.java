...
import de.quinscape.automaton.runtime.scalar.*;
...
public class FilterTransformer
{
    ...
    private Object transformRecursive(
            ...
            Map<String, Object> node, Field<?> conditionLeftSideField
    )
    {
        if (node == null)
        {
        ...
        switch (nodeType)
        {
            case COMPONENT:
            {
                ...
                return transformRecursive(resolver, fieldResolver, kid, conditionLeftSideField);
            }
            case CONDITION:
            {
                ...
                if (name.equals("and"))
                {
                    final List<? extends Condition> conditions = transformOperands(resolver, fieldResolver, operands, conditionLeftSideField);
            ...
            case VALUE:
            {
                ...
                if (value instanceof ComputedValueScalar)
                {
                    ...
                    return invokeComputedValue(conditionLeftSideField, scalar.getName(), scalar.getArgs());
                }
    ...
    private Object invokeComputedValue(Field<?> conditionLeftSideField, String name, List<GenericScalar> args) {
        ComputedValue computedValue = computedValues.get(name);
        return computedValue.evaluate(name, args, conditionLeftSideField);
    }
    ...
    private Object invokeFieldMethod(
        ...
        if (!(value instanceof Field)) {
            throw new AutomatonException("Field Operation: First operand did not evaluate to field: " + JSON_GEN.forValue(
                    condition));
        }
